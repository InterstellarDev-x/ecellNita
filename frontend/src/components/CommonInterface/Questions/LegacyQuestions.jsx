...
import { useContinueChat } from "../../../hooks/useChatQueries";
...
function QuestionCard({ item, audience }) {
  const continueChat = useContinueChat();
  const navigate = useNavigate();
  const openChat = async () => {
    try { const thread = await continueChat.mutateAsync(item._id); navigate(`/${audience}/questions?chat=${thread._id}`); }
    catch (error) { toast.error(error.message || "Could not open chat"); }
  };
    ...
    <article className="private-questions__card">
      <header>
        ...
        <div className="private-questions__header-actions">
          <button type="button" className="private-questions__unsend" onClick={openChat} disabled={continueChat.isPending}><MessageCircle size={13} /> Continue in chat</button>
